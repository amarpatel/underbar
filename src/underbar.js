...
(function() {
  ...
  _.each = function(collection, iterator) {
    if (Array.isArray(collection) === true && collection.length !== undefined) {
  ...
  _.map = function(array, iterator) {
    ...
    _.each(array,function(element){
      return results.push(iterator(element));
    });
    return results;
  };
  ...
  _.invoke = function(collection, functionOrKey, args) {
    if (typeof(functionOrKey) === 'function') {
      return _.map(collection,function(value){return functionOrKey.apply(value,args)});
    } else {
  ...
  _.shuffle = function(array) {
    var shuf = _.map(array,function(el){return el});
    for (var i=0;i<array.length;i++) {
      shuf.push(shuf.splice(Math.floor(Math.random()*(shuf.length-i)),1)[0]);
    }
    return shuf;
  };
